  ...
  const socialLinks = [
    {
      ...
      svg: `<svg class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
...
<footer class="bg-white/70 backdrop-blur-sm border-t border-white/20 py-4 mt-8">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center">
    <div class="flex space-x-4 mb-2">
      {#each socialLinks as link}
        <a href={link.url} target="_blank" aria-label={link.name} class="text-[rgb(29,73,167)]" rel="noopener noreferrer">
          {@html link.svg}
        </a>
      {/each}
    </div>
    <p class="text-sm text-gray-600 font-aileron">&copy; {new Date().getFullYear()} Aerogen Inc. All rights reserved.</p>
  </div>
</footer> 