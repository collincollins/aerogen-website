<script lang="ts">
  import { showContentToggle, keepNavbarVisible, airplaneContrailToggle } from './stores/devTools';
  ...
  // toggle airplane contrails
  const handleToggleAirplaneContrail = () => {
    if (!dev) return; // no-op in production
    
    airplaneContrailEnabled = !airplaneContrailEnabled;
    airplaneContrailToggle.set(airplaneContrailEnabled);
  };
...
{#if showDevTools}
  <div class="fixed bottom-4 right-4 z-50 flex flex-col gap-2 items-end">
    ...
    <!-- Airplane Contrail Toggle Button -->
    <button
      class={airplaneContrailEnabled ? "bg-accent/90 hover:bg-accent text-white px-4 py-2 rounded-full shadow-lg backdrop-blur-sm transition-all duration-300 flex items-center gap-2 text-sm" : "bg-primary/90 hover:bg-primary text-white px-4 py-2 rounded-full shadow-lg backdrop-blur-sm transition-all duration-300 flex items-center gap-2 text-sm"}
      on:click={handleToggleAirplaneContrail}
      on:keydown={(e) => e.key === 'Enter' && handleToggleAirplaneContrail()}
      aria-label={airplaneContrailEnabled ? "Disable airplane contrails" : "Enable airplane contrails"}
      tabindex="0"
    >
      <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" />
      </svg>
      {airplaneContrailEnabled ? "Airplane Contrails On" : "Airplane Contrails Off"}
    </button>
  </div>
{/if}
